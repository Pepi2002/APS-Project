...
class AccreditationAuthority:
    """Ente di Accreditamento per certificare le università"""

    def __init__(self, name: str, jurisdiction: str, authority_type: str):
        self.name = name
        self.jurisdiction = jurisdiction
        self.authority_type = authority_type
        self.accredited_institutions = {}
        self.accreditation_certificates = {}
        self.csprng = CSPRNGGenerator()

        # Genera chiavi per l'ente di accreditamento
    ...
    def accredit_university(self, university_did: str, university_name: str,
                            accreditation_level: str, valid_until: str) -> Dict[str, Any]:
        """Certifica una università emettendo un certificato di accreditamento"""
        ...
        certificate_data = {
            ...
            "accreditation_authority": self.name,
        ...
        # Memorizza il certificato
        self.accreditation_certificates[certificate_id] = certificate_data
        self.accredited_institutions[university_did] = certificate_id

        print(f"🏛️  Università '{university_name}' accreditata da {self.name}")
        print(f"📜 Certificato ID: {certificate_id}")
        print(f"🏆 Livello: {accreditation_level}")
        print(f"⏰ Valido fino al: {valid_until}")

        return certificate_data

    def verify_accreditation(self, university_did: str) -> bool:
        """Verifica se una università è accreditata"""
        if university_did not in self.accredited_institutions:
            return False

        certificate_id = self.accredited_institutions[university_did]
        certificate = self.accreditation_certificates[certificate_id]

        # Verifica la validità temporale
        valid_until = datetime.fromisoformat(certificate["valid_until"])
        if datetime.now() > valid_until:
            print(f"❌ Accreditamento scaduto per {university_did}")
            return False

        return True

    def get_accreditation_info(self, university_did: str) -> Optional[Dict[str, Any]]:
        """Ottiene le informazioni di accreditamento per una università"""
        if university_did in self.accredited_institutions:
            certificate_id = self.accredited_institutions[university_did]
            return self.accreditation_certificates[certificate_id]
        return None


class EnhancedDIDManager:
    """Gestore DID con simulazione blockchain e supporto per enti di accreditamento"""

    def __init__(self):
        self.did_registry = {}
        self.blockchain_blocks = []
        self.current_block = []
        self.accreditation_authorities = {}  # Modificato per contenere gli enti
        self.csprng = CSPRNGGenerator()

        # Crea gli enti di accreditamento principali
        self._initialize_accreditation_authorities()

    def _initialize_accreditation_authorities(self):
        """Inizializza gli enti di accreditamento europei"""
        print("🏛️  Inizializzazione Enti di Accreditamento...")

        # ENQA - European Network for Quality Assurance in Higher Education
        enqa = AccreditationAuthority("ENQA", "European Union", "Quality Assurance")
        self.accreditation_authorities["ENQA"] = enqa

        # ANVUR - Agenzia Nazionale di Valutazione del Sistema Universitario (Italia)
        anvur = AccreditationAuthority("ANVUR", "Italy", "National Agency")
        self.accreditation_authorities["ANVUR"] = anvur

        # HCERES - Haut Conseil de l'évaluation de la recherche (Francia)
        hceres = AccreditationAuthority("HCERES", "France", "National Agency")
        self.accreditation_authorities["HCERES"] = hceres

        print("✅ Enti di accreditamento inizializzati")

    def create_did(self, entity_name: str, entity_type: str,
                   public_key: RSAPublicKey,
                   accreditation_authority: str = None) -> str:
        """Crea DID con verifica di accreditamento per le università"""
        ...
        # Se è una università, verifica l'accreditamento
        if entity_type == "university" and accreditation_authority:
            self._handle_university_accreditation(did, entity_name, accreditation_authority)

        print(f"✅ DID creato: {did}")
        return did

    def _handle_university_accreditation(self, university_did: str, university_name: str,
                                         authority_name: str):
        """Gestisce l'accreditamento di una università"""
        if authority_name in self.accreditation_authorities:
            authority = self.accreditation_authorities[authority_name]

            # Determina il livello di accreditamento e la validità
            accreditation_level = "Full Accreditation"
            valid_until = (datetime.now() + timedelta(days=1825)).isoformat()  # 5 anni

            # Emette il certificato di accreditamento
            certificate = authority.accredit_university(
                university_did, university_name, accreditation_level, valid_until
            )

            # Aggiorna il documento DID con le informazioni di accreditamento
            self.did_registry[university_did]["accreditation"] = {
            ...
            print(f"⚠️  Ente di accreditamento '{authority_name}' non riconosciuto")

    def verify_university_accreditation(self, university_did: str) -> bool:
        """Verifica l'accreditamento di una università"""
        if university_did not in self.did_registry:
            return False

        did_document = self.did_registry[university_did]

        if "accreditation" not in did_document:
            print(f"❌ Università {university_did} non ha accreditamento")
            return False

        authority_name = did_document["accreditation"]["authority"]
        if authority_name not in self.accreditation_authorities:
            print(f"❌ Ente di accreditamento {authority_name} non riconosciuto")
            return False

        authority = self.accreditation_authorities[authority_name]
        is_valid = authority.verify_accreditation(university_did)

        if is_valid:
            print(f"✅ Università {university_did} correttamente accreditata da {authority_name}")
        else:
            print(f"❌ Accreditamento non valido per {university_did}")
...
class EnhancedErasmusCredentialSystem:
    ...
    def _generate_asymmetric_keys(self):
        """Algoritmo di Generazione delle Chiavi Asimmetriche (Gen K) con accreditamento"""
        ...
        # Crea i DID per le entità CON accreditamento
        self.host_did = self.did_manager.create_did(
            "Université de Rennes", "university", self.host_public_key, "HCERES"
        )
        self.home_did = self.did_manager.create_did(
            "Università di Salerno", "university", self.home_public_key, "ANVUR"
        )
        self.student_did = self.did_manager.create_did(
            "Mario Rossi", "student", self.student_public_key
        )

        print("✅ Chiavi asimmetriche generate e DID creati con accreditamento")
    ...
    def verify_verifiable_presentation(self, encrypted_package: Dict[str, str]) -> bool:
        """Verifica completa della Verifiable Presentation con controllo accreditamento"""
        ...
        try:
            ...
            # Fase 3.1: NUOVA - Verifica dell'accreditamento dell'università emittente
            print("3.1. 🏆 Verifica dell'accreditamento dell'università emittente...")
            if not self.did_manager.verify_university_accreditation(self.host_did):
                print("❌ ERRORE: L'università emittente non è correttamente accreditata!")
                return False
            print("✅ Università emittente correttamente accreditata")
            ...
            print("🏆 Accreditamento dell'università verificato")
            return True
